...
contract BasicDex {
  ...
  event TokenSwap(address _user, uint256 _tradeDirection, uint256 _tokensSwapped, uint256 _tokensReceived);
  event LiquidityProvided(address _user, uint256 _liquidityMinted, uint256 _creditTokenAdded, uint256 _assetTokenAdded);
  ...
  function assetPrice(uint256 creditIn) external view returns (uint256 assetOut) {
  ...
  function creditPrice(uint256 assetIn) external view returns (uint256 creditOut) {
    uint256 assetReserves = assetToken.balanceOf(address(this));
  ...
  function creditToAsset(uint256 tokensIn, uint256 minTokensBack) public returns (uint256 tokenOutput) {
    ...
    bool creditTokenTransferred = creditToken.transferFrom(msg.sender, address(this), tokensIn);
    if (!creditTokenTransferred) revert TokenTransferError(address(creditToken));
  ...
  function assetToCredit(uint256 tokensIn, uint256 minTokensBack) public returns (uint256 tokenOutput) {
    ...
    bool assetTokenTransferred = assetToken.transferFrom(msg.sender, address(this), tokensIn);
    if (!assetTokenTransferred) revert TokenTransferError(address(assetToken));
  ...
  function deposit(uint256 creditTokenDeposited) public returns (uint256 liquidityMinted) {
    ...
    uint256 creditTokenReserve = creditToken.balanceOf(address(this));
    uint256 assetTokenReserve = assetToken.balanceOf(address(this));
    uint256 assetTokenDeposited = (creditTokenDeposited * creditTokenReserve) / assetTokenReserve;
    ...
    // transfer credit tokens from user to dex
    bool creditTokenTransferred = creditToken.transferFrom(msg.sender, address(this), creditTokenDeposited);
    if (!creditTokenTransferred) revert TokenTransferError(address(creditToken));

    // transfer asset tokens from user to dex
    bool assetTokenTransferred = assetToken.transferFrom(msg.sender, address(this), assetTokenDeposited);
    if (!assetTokenTransferred) revert TokenTransferError(address(assetToken));

    emit LiquidityProvided(msg.sender, liquidityMinted, creditTokenDeposited, assetTokenDeposited);
  }
  ...
  function withdraw(uint256 amount) public returns (uint256 creditTokenAmount, uint256 assetTokenAmount) {
    if (liquidity[msg.sender] < amount) revert InsufficientLiquidityError(liquidity[msg.sender]);
